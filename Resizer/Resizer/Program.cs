...
namespace Resizer
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // 1. Skala om en bild beroende på angiven breddparameter, t.ex. 512px
        ...
        static void Run(Options options)
        {
            ...
            foreach (var filepath in files)
            {
                ...
                {
                    using (var outStream = File.OpenWrite(GetOutputFileName(filepath)))
                    {
                        ...
                        {
                            job.Decode(stream, false).Distort((uint)options.Width, (uint)options.Height, hints)
                                .SaturationSrgb(-100)
